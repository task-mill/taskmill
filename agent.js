#!/usr/bin/env node
/**
 * Taskmill Agent — a simple heartbeat loop that picks up issues and works on them.
 *
 * Usage: node agent.mjs [--api http://localhost:3005] [--agent-id a3] [--interval 10]
 */

var API = 'http://localhost:3005/db/taskmill'
var AGENT_ID = 'a3'  // Engineer by default
var INTERVAL = 15    // seconds between heartbeats

// Parse CLI args
process.argv.forEach(function(arg, i) {
  if (arg === '--api' && process.argv[i + 1]) API = process.argv[i + 1] + '/db/taskmill'
