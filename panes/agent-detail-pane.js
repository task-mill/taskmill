...
export default {
  ...
  render(subject, lionStore, container, context) {
    ...
    var issues = data.issues.filter(function(i) { return i.assigneeAgentId === agent.id })
          ...
          <div>
            ...
            <div class="card">
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 13px">
                <div>
                  <span class="text-muted text-xs">Heartbeat</span>
                  <div class="text-fg2">${heartbeat && heartbeat.enabled ? 'Every ' + (heartbeat.intervalSec / 60) + 'm' : 'Disabled'}</div>
                </div>
                <div>
                ...
                <div>
                  <span class="text-muted text-xs">Max concurrent runs</span>
                  <div class="text-fg2">${heartbeat && heartbeat.maxConcurrentRuns || '—'}</div>
                </div>
                ${agent.permissions && agent.permissions.canCreateAgents ? html`<div style="grid-column: span 2"><span class="badge badge-purple">Can hire agents</span></div>` : ''}
